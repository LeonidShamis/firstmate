...
make_case() {
  ...
  # These cases drive the real markdown backlog flow, so the case home pins
  # markdown storage rather than following the tracked default (tests/lib.sh).
  fm_test_markdown_tasks_toml "$case_dir"
...
seed_backlog_in_flight() {
  ...
  (cd "$case_dir" && tasks-axi add task-x1 "teardown fixture task" --kind "$kind" \
    --file "$case_dir/data/backlog.md" >/dev/null)
  (cd "$case_dir" && tasks-axi start task-x1 --file "$case_dir/data/backlog.md" >/dev/null)
}

backlog_row_state() {
  local case_dir=$1
  (cd "$case_dir" && tasks-axi show task-x1 --file "$case_dir/data/backlog.md" 2>/dev/null) |
