...
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout worktree isolation and fresh-base refusal rules, including spawns from linked homes and from a repository with no remote at all, which a `local-only` project may be.
...
It also proves the recorded worktree is still this task's before killing anything in it or returning it, refusing loudly when the records bind it to another task, and a rerun after a partial failure skips a worktree already returned instead of repeating the return.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, pre-teardown run conclusion, stale-lock recovery procedure, worktree-ownership evidence, and rerun-convergence record.
