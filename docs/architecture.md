...
A repository with no remote at all, which a `local-only` project may be, instead resets to the shared checkout's own recognised default branch and is refused when none resolves.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, worktree-ownership evidence, and rerun-convergence record.
