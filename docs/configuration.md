...
The tracked `[beads]` section names that one database twice on purpose: `dir = "data"` is the workspace tasks-axi resolves the graph from, and `path = "data/.beads"` is the graph directory firstmate's own migrated-hold reader exports as `BEADS_DIR`, so both readers must keep pointing at the same store.
[`tests/fm-captain-hold-lifecycle.test.sh`](../tests/fm-captain-hold-lifecycle.test.sh) pins that pairing by running a migrated-hold scan against a real graph in a home seeded from this tracked file.
Editing `.tasks.toml` back to `backend = "markdown"` restores the classic hand-editable markdown backlog with identical firstmate behavior, through the retained `[markdown]` settings: `data/backlog.md`, `done_keep = 10`, and an archive at `data/done-archive.md`.
...
That refusal reads the storage the home's own `.tasks.toml` declares, so a `TASKS_AXI_BACKEND` override in the calling shell cannot unlock a handoff, a remote outbox staging, or a receipt on a beads home.
[`tests/fm-backlog-beads-storage-guard.test.sh`](../tests/fm-backlog-beads-storage-guard.test.sh) pins all three refusals against an exported and an empty override.
`bin/fm-home-seed.sh` clones this repository, so a seeded secondmate home inherits this tracked configuration while the gitignored beads graph under `data/` stays behind.
Pin that home's `.tasks.toml` to `backend = "markdown"`, or create its own graph with `bd init` in its `data/`, before working in it: until then every tasks-axi backlog call there fails with `no beads database found`, and non-forced scout teardown refuses on a captain-call completion gate it cannot verify.
